...
2 Big differences: (1) this example has a custom controller in <color=cyan>Assets/WebPlayerTemplates/HappyFunTimes/controllers/mulit-machine/controller.html</color>.
That controller shows one example of using an avatar on the phone. (2) Each player is controlled by the <color=cyan>Assets/HappyFunTimes/MoreSamples/MultiMachineBirdScript.cs</color>.
That script does its own communication with the controller for all inputs and outputs. When a player goes off the left of the screen the code calls <color=cyan>SwitchGame</color>.
<color=cyan>SwitchGame</color>Sends the player and a bunch of state to another game by id and kills the player in the current game. From the receiving game's POV a new player
